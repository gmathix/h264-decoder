...
static always_inline uint32_t read_u(BitReader *br, int n) {
    return bitreader_read_bits(br, n);
}

/* parsing process described in §9.1 */
static always_inline uint32_t read_ue(BitReader *br) {
...
static always_inline int32_t read_se(BitReader *br) {
    int32_t ue = (int32_t) read_ue(br);

    return (-1 + 2*(ue & 1)) * (ue+1) / 2;
}

static always_inline uint32_t read_te(BitReader *br, int max) {
    if (max == 1) return !read_u(br, 1);
    return read_ue(br);
}

static always_inline uint32_t map_coded_block_pattern(uint32_t codeNum, int chromaArrayType, int isIntra) {
