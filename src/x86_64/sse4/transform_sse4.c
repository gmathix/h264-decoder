...
#define TRANSPOSE(l0, l1, l2, l3, temp0, temp1, temp2, temp3)  \
...
static always_inline void idct_4x4_sse(__m128i *r01, __m128i *r23, uint8_t *dst, int stride, int bitDepth) {
    ...
    TRANSPOSE(r0, r1, r2, r3, temp0, temp1, temp2, temp3);
    ...
    TRANSPOSE(f0, f1, f2, f3, temp0, temp1, temp2, temp3);
...
static void scaling_residual_4x4_lshift_avx(int qp, __m256i *r, __m256i scale) {
      int16_t shift = qp/6-4;
      __m128i shift_reg = _mm_loadu_si16(&shift);

      *r = _mm256_mullo_epi16(*r, scale);
      *r = _mm256_sll_epi16(*r, shift_reg);
}

static void scaling_residual_4x4_rshift_min_avx(int qp, __m256i *r, __m256i scale) {
      int16_t shift = 4-qp/6;

      __m256i add_reg = _mm256_set1_epi16(1 << (3-qp/6));
      __m128i shift_reg = _mm_loadu_si16(&shift);

      *r = _mm256_mullo_epi16(*r, scale);
      *r = _mm256_add_epi16(*r, add_reg);
      *r = _mm256_sra_epi16(*r, shift_reg);
}

static void print_reg(__m128i *reg) {
      for (int i = 0; i < 8; i++) {
            printf("%d ", ((int16_t*)reg)[i]);
      }
      printf("\n");
}
